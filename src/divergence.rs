...
impl DivergenceMonitor {
    ...
    pub fn observe(&mut self, loss: f32, gradient_norm: f32) {
        let alpha = if self.num_observations == 0 { 1.0 } else { 1.0 - self.ema_decay };
        
        // Update loss EMA and variance
        let loss_delta = loss - self.loss_ema;
        self.loss_ema += alpha * loss_delta;
        self.loss_var_ema = self.ema_decay * self.loss_var_ema 
            + alpha * loss_delta * loss_delta;
        ...
        self.gradient_var_ema = self.ema_decay * self.gradient_var_ema
            + alpha * grad_delta * grad_delta;
    ...
    pub fn check(
        ...
        let vanishing = state.gradient_norm < self.config.vanishing_gradient_threshold 
            * self.baseline_gradient_norm;
        ...
        let vanish_signal = SignalResult {
            ...
            deviation_sigma: 0.0,
            triggered: vanishing,
            severity: if vanishing { DivergenceLevel::Warning } else { DivergenceLevel::Normal },
        };
        ...
        // Check NaN/Inf
        if state.loss.is_nan() || state.loss.is_infinite() 
            || state.gradient_norm.is_nan() || state.gradient_norm.is_infinite() 
        {
    ...
    fn generate_summary(&self, signals: &[SignalResult], severity: DivergenceLevel) -> String {
        let triggered: Vec<_> = signals.iter()
        ...
        match severity {
            ...
            DivergenceLevel::Warning => {
                format!("Warning: {} triggered divergence detection", triggered.join(", "))
            }
            DivergenceLevel::Critical => {
                format!("Critical: {} indicate imminent training failure", triggered.join(", "))
            }
        }
    }
