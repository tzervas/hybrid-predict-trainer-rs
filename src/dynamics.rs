...
#[cfg(all(feature = "cuda", feature = "candle"))]
use crate::gpu::kernels::rssm_rollout::{rssm_rollout_gpu, RssmEnsembleWeights, RssmRolloutConfig};
...
impl RSSMLite {
    ...
    pub fn predict_y_steps(
        ...
        // GPU-accelerated path (when CUDA available and enabled)
        #[cfg(all(feature = "cuda", feature = "candle"))]
        {
            // Try GPU first, fall back to CPU if it fails
            if let Ok(result) = self.predict_y_steps_gpu(state, y_steps) {
                return result;
            }
            // GPU failed, continue to CPU path below
            tracing::warn!("GPU rollout failed, falling back to CPU");
        }
    ...
    /// Converts GRU weights to GPU format for GPU-accelerated rollout.
    #[cfg(all(feature = "cuda", feature = "candle"))]
    fn to_gpu_weights(&self) -> RssmEnsembleWeights {
        let gpu_gru_weights: Vec<GpuGruWeights> = self
            .gru_weights
            .iter()
            .map(|weights| GpuGruWeights {
                w_z: weights.w_z.clone(),
                u_z: weights.u_z.clone(),
                b_z: weights.b_z.clone(),
                w_r: weights.w_r.clone(),
                u_r: weights.u_r.clone(),
                b_r: weights.b_r.clone(),
                w_h: weights.w_h.clone(),
                u_h: weights.u_h.clone(),
                b_h: weights.b_h.clone(),
        ...
        RssmEnsembleWeights {
            gru_weights: gpu_gru_weights,
            loss_head_weights: self.loss_head_weights.clone(),
            delta_head_weights: self.weight_delta_head_weights.clone(),
        }
    }

    /// GPU-accelerated predict_y_steps when CUDA is available.
    ///
    /// Uses GPU RSSM rollout kernel for 5-50× speedup on larger models.
    /// Falls back to CPU if GPU fails.
    #[cfg(all(feature = "cuda", feature = "candle"))]
    fn predict_y_steps_gpu(
        &self,
        state: &TrainingState,
        y_steps: usize,
    ) -> HybridResult<(PhasePrediction, PredictionUncertainty)> {
        ...
        // Prepare GPU config
        let gpu_config = RssmRolloutConfig {
            ensemble_size: self.config.ensemble_size,
            hidden_dim: self.config.deterministic_dim,
        ...
        // Prepare initial latent states (deterministic only for now)
        let initial_latents: Vec<Vec<f32>> = self
            .latent_states
            .iter()
            .map(|latent| latent.deterministic.clone())
            .collect();

        let features = state.compute_features();
        let initial_loss = state.loss;

        // GPU rollout
        let results = rssm_rollout_gpu(&gpu_config, &gpu_weights, &initial_latents, &features, initial_loss)?;

        // Aggregate ensemble predictions (same as CPU path)
        let ensemble_size = results.len() as f32;

        // Build mean trajectory
        let trajectory_length = y_steps + 1;
        let mut mean_trajectory = vec![0.0; trajectory_length];
        for result in &results {
            for (i, &loss_val) in result.trajectory.iter().enumerate() {
                mean_trajectory[i] += loss_val / ensemble_size;
            }
        }

        let predicted_final_loss = mean_trajectory[y_steps];

        // Compute epistemic uncertainty (ensemble variance)
        let final_losses: Vec<f32> = results.iter().map(|r| r.trajectory[y_steps]).collect();
        let variance: f32 = final_losses
            .iter()
            .map(|&l| (l - predicted_final_loss).powi(2))
            .sum::<f32>()
            / ensemble_size;
        let epistemic = variance.sqrt();

        // Aleatoric uncertainty (average trajectory variance)
        let mut aleatoric = 0.0_f32;
        for step_idx in 0..trajectory_length {
            let step_mean = mean_trajectory[step_idx];
            let step_variance: f32 = results
                .iter()
                .map(|r| (r.trajectory[step_idx] - step_mean).powi(2))
                .sum::<f32>()
                / ensemble_size;
            aleatoric += step_variance;
        }
        aleatoric = (aleatoric / trajectory_length as f32).sqrt();

        let total_uncertainty = (epistemic.powi(2) + aleatoric.powi(2)).sqrt();

        // Compute uncertainty-adjusted confidence
        let base_confidence = 1.0 / (1.0 + total_uncertainty);
        let trajectory_std = epistemic;
        let relative_uncertainty = trajectory_std / predicted_final_loss.abs().max(0.1);
        let confidence = (base_confidence * (1.0 - relative_uncertainty.min(0.5))).clamp(0.0, 1.0);

        // Decode weight delta from ensemble mean of final combined states
        let mean_final_combined: Vec<f32> = results[0]
            .final_combined
            .iter()
            .enumerate()
            .map(|(i, _)| {
                results
                    .iter()
                    .map(|r| r.final_combined[i])
                    .sum::<f32>()
                    / ensemble_size
            })
            .collect();

        let weight_delta = self.decode_weight_delta(&mean_final_combined, state, &mean_trajectory, y_steps, confidence);
        ...
        let prediction = PhasePrediction {
            ...
            confidence,
            loss_bounds: (
                predicted_final_loss - 2.0 * trajectory_std,
                predicted_final_loss + 2.0 * trajectory_std,
            ),
            num_steps: y_steps,
        };
        ...
        tracing::debug!(
            y_steps = y_steps,
            predicted_loss = %predicted_final_loss,
            confidence = %confidence,
            "GPU rollout complete"
        );
